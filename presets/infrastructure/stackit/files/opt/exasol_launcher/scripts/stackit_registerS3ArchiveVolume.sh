...
ARCHIVE_ENABLED="$(infra_jq -er '.stackit.archive.enabled')"
if [[ "${ARCHIVE_ENABLED}" != "true" ]]; then
  log_substep_info "Archive setup disabled; skipping registration"
  exit 0
fi

bucket_id="$(infra_jq -er '.stackit.archive.bucketId')"
s3_endpoint="$(infra_jq -er '.stackit.archive.s3Endpoint')"
archive_volume_name="$(infra_jq -er '.stackit.archive.volumeName')"
access_key="$(infra_jq -er '.stackit.archive.accessKey')"
secret_key="$(infra_jq -er '.stackit.archive.secretKey')"
