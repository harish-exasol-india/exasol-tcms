...
locals {
  ...
  infrastructure_payload = {
    ...
    # directly to the user-data limit (StackIT does not document what it is).
    ...
    postInstall = {
      ...
      scripts = var.s3_archive_enabled ? ["/opt/exasol_launcher/scripts/stackit_registerS3ArchiveVolume.sh"] : []
    }

    # Cloud-provider specific values needed by optional infra hooks.
    stackit = {
      archive = var.s3_archive_enabled ? {
        enabled    = true
        bucketId   = local.archive_bucket_id
        volumeName = "default_archive"
        s3Endpoint = stackit_objectstorage_bucket.remote_archive[0].url_virtual_hosted_style
        accessKey  = stackit_objectstorage_credential.credential[0].access_key
        secretKey  = stackit_objectstorage_credential.credential[0].secret_access_key
        } : {
        enabled = false
      }
    }
  ...
  node_payload_by_name = {
    for n in local.nodes : n.name => {
      ...
      # On StackIT, virtio truncates UUIDs to 20 chars in ID_SERIAL.
      hostDatadisk      = "/dev/exasol_data_01"
      hostDatadiskMatch = "ENV{ID_SERIAL}==\"${substr(try(stackit_volume.data_disks[n.name].volume_id, ""), 0, 20)}\""
    }
  }
}
